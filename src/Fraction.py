...
class Fraction:
    ...
    def __init__(self, numerator, denominator=1):
        ...
        if isinstance(numerator, str):
            if "/" in numerator:
                num, den = numerator.split("/") #to split the numerator and denominator
                numerator, denominator = int(num.strip()), int(den.strip())# to get rid of the white spaces
            else:
                numerator, denominator = int(numerator.strip()), 1 # if only numerator is provided
